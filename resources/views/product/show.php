...
<?php else: ?>
    <section>
        <article>
            ...
            <p>Catégorie : <?= $productId['name']; ?> - Plus que <?= $productId['stock']; ?> en stock. Poids : <?= $productId['weight']; ?> kilos.</p>
            <p>Déscription : <?= $productId['description']; ?></p>
            <p name="id"> Ref : <?= $productId['id'];?></p>
                <p>HT = <?= $productId['ht']; ?>€ + TVA = <?= $productId['tva']; ?>€ = <?= $productId['ttc']; ?>€ TTC </p>
